...
server, or use optional PaddleOCR in a separate persistent worker environment. The
...
| OCR reader | llama.cpp/Ollama with a vision model, or the optional local PaddleOCR environment below |
...
Click a box to read its text, category, coordinates, and Paddle recognition confidence where
...
**It is enabled only when the run actually returned boxes**: every Paddle line does, as does a
...
one, **Cancel** it. Paddle cancellation terminates its active worker and the next Paddle job
...
do not make Paddle parallel: all Paddle entries share one persistent serialized worker so one
model copy stays resident, and waiting entries say so in their stage.
...
| `server`, `backend`, `model` | which endpoint and model actually ran it; Paddle records `local`, `paddleocr`, and its recognizer |
...
| `ocr_profile` | `typhoon`, `dots`, or `paddle` — the pass-1 shape/reader that read the page, taken from the pages themselves. Blank on rows written before profiles existed, and on `run_type=extract` rows, which read no page |
| `ocr_lines`, `ocr_confidence`, `ocr_device`, `ocr_version`, `paddle_version`, `ocr_detector` | Paddle-only provenance: line count, mean recognition confidence (0–1), device, PaddleOCR version, PaddlePaddle version, and detector. Blank for model-server reads; token/prefill/decode columns are blank on Paddle rows because Paddle is not a generative model |
...
and `paddle`. Add `?probe=1` to import the isolated Paddle runtime and verify it.

`POST /api/ocr/reader` — `{"reader":"paddle","probe":true}` selects a reader
and optionally verifies it. Workspace and Queue requests may override the process
default by sending multipart field `reader=server|paddle`; the chosen reader is
...
- Run `python -m unittest discover -s tests -v` for the portable Paddle adapter and API
