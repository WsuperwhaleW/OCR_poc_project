...
**There are two reading pipelines and they are two tabs on the page.**

| Tab | Pass 1 | Pass 2 |
|---|---|---|
| **LLM pipeline** | the model server reads the page: llama.cpp or Ollama over HTTP, with a prompt, a sampler and a token budget | the same model server |
| **Python library** | PaddleOCR or EasyOCR reads it in this machine's own processes: a detector and a recognizer, no prompt and no tokens | still the model server |

Pass 2 is not split, and the Python library tab says so rather than hiding it: mapping a
transcript onto a form is a text task and no OCR library does it, so a library run still
sends its transcript to a model server. That is why the endpoint and the extraction model
appear on both tabs.
...
The pipeline, the reading library and the model server are all switchable from the page
while it runs.
...
Open the **Python library** tab and choose **Local PaddleOCR** under Reading library. Queue entries remember the
...
Open the **Python library** tab and choose **Local EasyOCR** under Reading library, or run:
...
Drop a file on either pipeline tab — **LLM pipeline** or **Python library** — pick one from
the folder list, or pick a benchmark case. Multi-page input is capped at 10 pages, uploads at 32 MB (`MAX_PAGES`,
`MAX_UPLOAD_MB`). PDF and HEIC can't render in an `<img>`, so the page shows a filename
card instead of a thumbnail.

**The drop zone, the Run button and the staged file are shared between the two pipeline
tabs**, and moved between them rather than duplicated: one file is staged at a time whichever
tab is showing, so a document can be read with the model server, switched to the other tab
and read again with PaddleOCR without being dropped twice. What is *not* shared is the
settings — a page-reading profile and a loop guard exist only on the LLM tab, because they
are properties of a model-server request and there is nothing for them to do on a library
read.
...
`--engine llm|library|both` chooses which pipeline's readers may be drawn; **`both` is the
CLI default**, because a sweep across the two is exactly the comparison the shared run log
exists for. The **Random test** pane instead follows whichever pipeline tab is showing — a
pane is where one pipeline is being worked on — and says so above the round list. Either
way the *extractor* pool is never narrowed: pass 2 runs on the model server under both
engines. Asking for a pipeline that has nothing installed is refused by name rather than
coming back as "no model reports vision".
