...
Drop a file on the **Workspace** tab. Multi-page input is capped at 10 pages, uploads at 32 MB (`MAX_PAGES`,
...
many it ignored, rather than silently discarding them.

**A dropped file is scored when it is recognised.** There is no benchmark-case picker here:
`POST /api/match` identifies the file by name, by an alias in `solution/manifest.json`, or
by the sha256 of its contents, and says so on the chip above the Run button before anything
is read. To read a case *by name*, use the **OCR only** tab.

For several documents, `python compare.py` reads and scores every benchmark case, and
`python randomtest.py` sweeps combinations.
...
`ocr` row whose pass-2 columns are blank *because nothing extracted*, which is a different
state from an extraction that returned nothing, and the run log keeps the two apart.

It is the mirror of **Fields only**, and the pair exist for one reason between them: a
field that came back wrong is otherwise unattributable. Fields only takes the read out of
the question by feeding the truth transcript; OCR only takes the extraction out by never
running it. Between them they isolate the two passes on the same document.
...
The **Contest between** controls were removed from the page on 2026-09-09. The planner is
untouched: `randomtest.py --contest` takes the ranking from [Standouts](#standouts) and runs
the top and bottom of it again **on the same documents, at one fixed Detail** (`medium`), with
the extraction shape the app is set to. The only thing that varies is the model.

That is the opposite of what a random plan does, and deliberately so: a random plan gives
...
and it is a deliberate, occasional sweep rather than something to press mid-session, which is
why it lives on the CLI now.

```bash
python randomtest.py http://localhost:5000 --contest --subject ocr_model --top 5 --documents 2
```

`--subject` is `ocr_model`, `extract_model`, `case` or `pipeline`; `--no-bottom` runs the
leaders alone. Only entries that have a **score**, and that this endpoint still serves, enter:
anything ranked but unserved, and anything unranked, is reported and skipped. A model in both
the top and the bottom slice is tagged as neither.
...
Every document read appends one row to `logs/runs.csv` — from the Workspace, from the
**OCR only** tab, from `/api/ocr`, and from `compare.py`. The **Run log** card at the bottom of the page shows the
...
left, beside Workspace, OCR only, Fields only and Random test. It draws every type this app can
